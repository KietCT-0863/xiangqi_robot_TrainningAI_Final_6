...
CÁC HÀM:
  ...
  move_safe_pose()       → Di chuyển tịnh tiến an toàn theo đường thẳng (Z không đổi): Dùng MoveCart
  movel_pose()           → Di chuyển thẳng đứng (Lên xuống kẹp): Dùng MoveCart
  go_to_idle_home()      → Về IDLE (config.IDLE_X/Y/Z)
  go_to_home_chess()     → Về điểm chuẩn đã dạy điểm "HOMECHESS" (Dùng MoveJ thực thụ để vung tay vòng cung chống kẹt trục)
...
FIX (2026-03-07): ĐỔI TÊN movej_pose -> move_safe_pose
  Hàm `movej_pose` cũ thực chất dùng lệnh `MoveCart` (Linear) để mũi kẹp chạy thẳng hàng dọc theo bàn cờ không bị xệ gầm quẹt trúng rác cờ khác. 
  Đã đổi tên hàm thành `move_safe_pose()` cho đỡ gây hiểu lầm. 
  Tính năng `MoveJ` thực thụ bằng nội suy Joint chỉ được dùng duy nhất ở điểm về nhà `HOMECHESS` nhằm sửa lỗi Singularity (Lỗi 101/14/112).
...
Chứa class `InputHandler`. Điều hướng thao tác người dùng Pygame vào `GameState` và `HardwareManager`. Là cầu nối trung tâm để xử lý toàn bộ tương tác giữa Người chơi và Hệ thống.

* CHỨC NĂNG BÀN PHÍM (`handle_keyboard`):
  1. Phím `SPACE` (Trigger YOLO Detection): Kích hoạt camera quét và ghi nhận nước đi của người trên bàn thật.
     - Lấy T2 snapshot từ `CameraMonitor`.
     - Nếu chưa có T1 baseline (mốc cơ sở), lệnh lấy T2 sẽ trở thành thao tác "chụp lấy mốc T1" thay vì tính nước đi. Cần thiết khi khởi động game hoặc sau lỗi.
     - So sánh T2 và T1 qua `YoloSnapshotDetector` để trích xuất ra nước đi `(src -> dst)`.
     - Validate luật cờ cẩn thận (giúp chống nhận diện rác). Nếu lỗi, kích hoạt chế độ `manual_override` (ép dùng chuột đánh trên màn hình). Nước đi hợp lệ sẽ đổi lượt gửi đến chờ AI.
  2. Phím `Z` (Rollback / Undo): Hoàn tác lại nước đi lỗi do AI hoặc Camera.
     - Xóa dữ liệu nước đi mới nhất.
     - Đặc biệt quan trọng: Khôi phục lại T1 baseline (ảnh mốc Camera) về đúng hình ảnh trước khi đánh.

* CHỨC NĂNG CHUỘT (`handle_mouse_down`):
  - Kéo thả cờ (Manual Override): Hoạt động khi chức năng Nhận diện bị lỗi hoặc chạy `config.DRY_RUN = True`. Hỗ trợ kéo thả quân cờ trực tiếp trên UI Pygame đi thay cho người. Sau khi kéo bù xong, tự động chụp một bức T1 baseline mới cho mượt hệ thống.
  - Buttons: Dò tọa độ Click xử lý Nút Nhượng Bộ (Surrender) hoặc Tạo ván mới (New Game).
